...
describe("status-mapping", () => {
  it("后端任务状态映射为中文文案", () => {
    ...
    expect(getStatusLabel("LINK_EXTRACTED")).toBe("链接已提取")
    expect(getStatusLabel("LINK_READY")).toBe("链接已就绪")
    ...
    expect(getStatusLabel("DRY_RUN")).toBe("演练完成")
  ...
  it("后端任务状态映射为状态颜色变量", () => {
    ...
    expect(getStatusColor("LINK_EXTRACTED")).toContain("--color-status-success")
    expect(getStatusColor("LINK_READY")).toContain("--color-status-success")
    ...
    expect(getStatusColor("DRY_RUN")).toContain("--color-status-warning")
