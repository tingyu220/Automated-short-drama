...
_ENV_USE_REAL = "WORKBUDDY_USE_REAL_ADAPTERS"
...
def build_adapters(
    ...
    """
    use_real_adapters = _resolve_use_real(use_real)
...
def _resolve_use_real(use_real: bool | None) -> bool:
    if use_real is not None:
        return use_real
    return _parse_bool_env(_ENV_USE_REAL, default=False)
...
def _parse_bool_env(name: str, *, default: bool) -> bool:
    ...
    if normalized in _FALSY:
        return False
    raise ConfigurationError(f"{name} 必须是严格布尔值，收到: {raw!r}")


def _required_env(name: str, label: str) -> str:
    value = os.getenv(name, "").strip()
    if not value:
        raise ConfigurationError(f"真实适配器缺少环境变量 {name}（{label}）")
    return value
